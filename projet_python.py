...
#demande le genre du nourrisson et le récupère danslavariable genre
...
#boucle de récupération du genre tant que différent de g ou f
while (genre != 'g' and genre != 'f'):
    genre = input ("Entrez le genre de votre nourrisson ('g' pour garçon, 'f' pour fille) : ")
if genre == 'f':
    libellesexe = 'une fille'
else:
    libellesexe = 'un garçon'


#fonction pour savoir si c'est bien un float qui a été saisie
def isnumber(s):
    try:
        float(s)
        return True
...
#boucle de récupération de l'âge tant que non compris entre 0 et 60
while (float(age) <0 or float(age)>60 ) :
    age = input ("veuillez entrer l'âge de votre nourrisson en mois (entre 0 et 60 mois) : ") 
    numericite = isnumber(age)
    if numericite == False:
        age =-1

#je mets un nombre entier pour l'âge
agentier =round(float(age))
print(agentier)

#récupération du poids de l'enfant
while float(poids) < 0:
    poids = input("Veuillez entrer le poids de votre nourrisson en kg : ") 
    numericite =isnumber (poids)
    if numericite == False:
        poids =-1
    
#récupération de la taille du nourrisson
while float(taille) < 0:
    taille = input("Veuillez entrer le taille de votre nourrisson en cm : ") 
    numericite =isnumber (taille)
    if numericite == False:
        taille =-1

#récupération de le périmètre cranien du nourrisson
while float(perimetre) < 0:
    perimetre = input("Veuillez entrer le périmètre cranien de votre nourrisson en cm : ") 
    numericite =isnumber (perimetre)
    if numericite == False:
        perimetre =-1
print(' ')
#gestion de la norme pour le poids,taille et perimetre en fonction du genre et de l'age
if genre == 'f':
    #min poids fille
    norme_poids_min = low_weights_girls[agentier]
    #max poids fille
    norme_poids_max = high_weights_girls[agentier]
    #min taille fille
    norme_taille_min = low_heights_girls[agentier]
    #max taille fille
    norme_taille_max = high_heights_girls[agentier]
    #min périmètre cranien fille
    norme_perim_min = low_skulls_girls[agentier]
    #max perimètre cranien fille
    norme_perim_max = high_skulls_girls[agentier]
else:
    #min poids garçon
    norme_poids_min = low_weights_boys[agentier]
    #max poids garçon
    norme_poids_max = high_weights_boys[agentier]
    #min taille garçon
    norme_taille_min = low_heights_boys[agentier]
    #max taille garçon
    norme_taille_max = high_heights_boys[agentier]
      #min périmètre cranien fille
    norme_perim_min = low_skulls_boys[agentier]
    #max perimètre cranien fille
    norme_perim_max = high_skulls_boys[agentier]
print (f'la norme de poids pour {libellesexe} de {age} mois est située entre {norme_poids_min} kg et {norme_poids_max} kg')
if float(poids) > norme_poids_min and float(poids) < norme_poids_max:
...
#affichage des normes et du résultat de norme pour le poids
print (f'la norme de poids pour {libellesexe} de {agentier} mois est située entre {norme_poids_min} kg et {norme_poids_max} kg')
if float(poids) > norme_poids_min and float(poids) < norme_poids_max:
    print(f'le poids de votre nourrisson ({poids}kg) est dans la norme!\n')
else:
     print(f'le poids de votre nourrisson ({poids}kg) n\'est pas dans la norme!\n')

#affichage des normes et du résultat de norme pour la taille
print (f'la norme de taille pour {libellesexe} de {agentier} mois est située entre {norme_taille_min} cm et {norme_taille_max} cm')
if float(taille) > norme_taille_min and float(taille) < norme_taille_max:
...
#affichage des normes et du résultat de norme pour le périmètre cranien
print (f'la norme de périmètre cranien pour {libellesexe} de {agentier} mois est située entre {norme_perim_min} cm et {norme_perim_max} cm')
if float(perimetre) > norme_perim_min and float(perimetre) < norme_perim_max:
    print(f'le périmètre cranien de votre nourrisson ({perimetre}cm) est dans la norme!')
else:
     print(f'le perimetre cranien de votre nourrisson ({perimetre}cm) n\'est pas dans la norme!')     